...
const Months = {
  ...
  /**
   * تعديل حدود شهر (تاريخ نزول راتبه وتاريخ الراتب القادم) مع تعديل الشهر السابق واللاحق
   * حتى لا يحدث تداخل أو فجوة بين الشهور. يعيد رسالة خطأ أو null.
   */
  setBounds(month, start, end) {
    const idx = state.months.indexOf(month);
    if (idx < 0) return 'الشهر غير موجود.';
    if (end < start) return 'تاريخ الراتب القادم يجب أن يكون بعد تاريخ نزول راتب هذا الشهر.';
    const len = DateUtils.diffDays(start, end) + 1;
    if (len > 62) return 'مدة الشهر المالي لا يمكن أن تزيد عن 62 يومًا.';
    const prev = state.months[idx - 1];
    const next = state.months[idx + 1];
    if (prev && DateUtils.addDays(start, -1) < prev.start) return `تاريخ البداية يجب أن يكون بعد ${DateUtils.long(prev.start)} (بداية الشهر السابق).`;
    if (next && DateUtils.addDays(end, 1) > next.end) return `تاريخ الراتب القادم يجب أن يكون قبل ${DateUtils.long(DateUtils.addDays(next.end, 1))}.`;
    month.start = start;
    month.end = end;
    if (prev) prev.end = DateUtils.addDays(start, -1);
    if (next) next.start = DateUtils.addDays(end, 1);
    return null;
  },

  /**
   * تسجيل نزول الراتب في تاريخ معين:
   *  - إذا كان التاريخ في أول أيام الشهر الحالي (راتب متأخر): تُنقل بداية الشهر لهذا التاريخ ويمتد الشهر السابق حتى اليوم الذي قبله.
   *  - غير ذلك (راتب مبكر أو في موعده): ينتهي الشهر الحالي قبل التاريخ بيوم ويبدأ شهر جديد.
   */
  salaryPlan(date) {
    const month = Data.monthForDate(date) || Data.currentMonth();
    if (!month) return null;
    if (date === month.start) return { type: 'same', month };
    const idx = state.months.indexOf(month);
    const prev = state.months[idx - 1];
    const late = !!prev && DateUtils.diffDays(month.start, date) <= 10 && date > month.start;
    if (late) return { type: 'late', month, prev };
    // نهاية الشهر الجديد = قبل موعد الراتب المتوقع التالي (بعد ما لا يقل عن أسبوعين)
    const nextPay = Periods.nextPayAfter(DateUtils.addDays(date, 14), state.settings.payday);
    return { type: 'new', month, end: DateUtils.addDays(nextPay, -1) };
  },

  applySalary(date, salary) {
    const plan = Months.salaryPlan(date);
    if (!plan) return 'لا يوجد شهر مالي.';
    if (plan.type === 'same') {
      plan.month.salary = salary;
      return null;
    }
    if (plan.type === 'late') {
      const err = Months.setBounds(plan.month, date, plan.month.end);
      if (err) return err;
      plan.month.salary = salary;
      if (plan.month.trackFrom && plan.month.trackFrom < date) plan.month.trackFrom = null;
      return null;
    }
    // شهر جديد
    const idx = state.months.indexOf(plan.month);
    if (state.months[idx + 1]) return 'يوجد شهر بعد هذا الشهر بالفعل؛ عدّل تواريخه من صفحة الشهور.';
    plan.month.end = DateUtils.addDays(date, -1);
    const m = Months.build(date);
    m.end = plan.end;
    m.salary = salary;
    if (state.months.some((x) => x.id === m.id)) m.id = Utils.uid('m_');
    state.months.push(m);
    Store.sortMonths();
    return null;
  },
...
const Calc = {
  ...
  /** التحويلات الخارجة من الحساب الرئيسي (للصناديق، للأموال الإضافية، أو سحب خارجي) مجمعة حسب اليوم */
  mainOutflowsByDay(month) {
    const map = {};
    state.transactions.forEach((t) => {
      if (t.type === 'transfer' && t.from === 'main' && t.date >= month.start && t.date <= month.end) {
        map[t.date] = Utils.round2((map[t.date] || 0) + t.amount);
      }
    });
    return map;
   ...
   *
   * المتاح للمصروف اليومي = الراتب − المصاريف الشهرية − المبلغ المستهدف
   *                         − المبالغ المحولة من الحساب الرئيسي للصناديق (من يوم التحويل)
   *                         + أي مبالغ أضافها المستخدم للميزانية
   * بهذا تبقى الميزانية مطابقة لرصيد الحساب الرئيسي الفعلي.
   *
  ...
  monthBudget(month) {
    return Calc.memo('budget:' + month.id, () => {
      ...
      const outMap = Calc.mainOutflowsByDay(month);
      const outflows = Utils.round2(Object.values(outMap).reduce((a, v) => a + v, 0));
      const grossPool = Utils.round2(month.salary + boosts - fixedTotal - month.retainTarget);
      const pool = Utils.round2(grossPool - outflows);
      const dailyPool = Math.max(0, pool);
      const distributable = Math.max(0, grossPool - priorSpent);
      ...
      for (let i = 0; i < totalDays; i += 1) {
        ...
        const out = outMap[date] || 0;
        let base;
        let allowance;

        // المبالغ المحولة للصناديق تُخصم من المتاح ابتداءً من يوم التحويل
        if (out > 0) {
          outSoFar += out;
          remainingPool -= out;
          if (carry > 0) {
            // يُستهلك الفائض المرحّل أولًا حتى لا يُخصم التحويل مرتين
            const fromCarry = Math.min(carry, out);
            carry -= fromCarry;
            remainingPool += fromCarry;
          }
        }
        ...
        if (mode === 'spread') {
          base = Math.max(0, (distributable - spentSoFar - outSoFar) / daysLeft);
          allowance = base;
        } else {
      ...
      return {
        ...
        perDayBase: Utils.round2(Math.max(0, distributable - outflows) / Math.max(1, trackedDays)),
        rows,
        mode,
        // المتبقي من الراتب هذا الشهر = الراتب − الثابتة − اليومية
        remaining: Utils.round2(month.salary + boosts - fixedTotal - outflows - dailySpent),
        // المتوقع في نهاية الشهر إذا التزم المستخدم بالميزانية
        expectedLeft: Utils.round2(month.salary + boosts - fixedTotal - outflows - Math.max(dailyPool, dailySpent)),
      };
    });
  },
        ...
        <div class="stack">
          <section class="card">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:14px">
              <p class="section-sub" style="margin:0">${DateUtils.range(month.start, month.end)} · الراتب القادم ${DateUtils.long(DateUtils.addDays(month.end, 1))}</p>
              <div style="display:flex;gap:6px">
                <button type="button" class="btn btn-secondary btn-sm" data-action="edit-month" data-id="${month.id}">${Utils.icon('edit', 'icon-sm')} تعديل التواريخ</button>
                <button type="button" class="btn btn-primary btn-sm" data-action="salary-arrived">${Utils.icon('coins', 'icon-sm')} نزل الراتب</button>
              </div>
            </div>
            ...
            ${b.outflows > 0 ? `<p class="field-hint" style="margin:-6px 0 12px">الراتب ${Utils.money(month.salary)} − المصاريف الشهرية ${Utils.money(b.fixedTotal)} − المستهدف ${Utils.money(month.retainTarget)} − المحوّل للصناديق ${Utils.money(b.outflows)}${b.boosts > 0 ? ` + إضافات ${Utils.money(b.boosts)}` : ''}</p>` : ''}
  ...
  months() {
    ...
    return `
      <div class="section-head" style="margin-top:4px">
        <p class="section-sub">كل شهر مالي يبدأ يوم نزول الراتب وينتهي قبل الراتب التالي بيوم (اليوم المتوقع: ${state.settings.payday}). إذا نزل الراتب في يوم مختلف اضغط «نزل الراتب». اضغط على أي شهر لرؤية التفاصيل أو تعديل تواريخه.</p>
  ...
  monthDetails(month) {
    ...
    return `
      <div class="stack">
        ...
        ${b.outflows > 0 ? `<div class="callout">${Utils.icon('transfer')}<p>حُوّل ${Utils.money(b.outflows)} من الحساب الرئيسي إلى الصناديق في هذا الشهر، وخُصم من ميزانية المصروف اليومي.</p></div>` : ''}
...
const Forms = {
  ...
  openTransfer({ mode, fundId = null }) {
    ...
    const body = `
      <form class="form" data-form="transfer" novalidate>
        ...
        ${fields}
        ${mode === 'extraMain' || mode === 'withdraw' ? `
          <label class="check-row" ${mode === 'withdraw' ? 'data-show-when-main' : ''}>
            <input type="checkbox" name="affectsBudget" value="1">
            <span><b>إضافة المبلغ إلى ميزانية المصروف اليومي لهذا الشهر${mode === 'withdraw' ? ' (عند السحب إلى الحساب الرئيسي)' : ''}</b><br><span class="muted">إذا لم تفعل ذلك سيزيد رصيد الحساب الرئيسي فقط دون تغيير الميزانية اليومية.</span></span>
          </label>` : ''}
  ...
  submitTransfer(form) {
    ...
    const affectsBudget = values.to === 'main' && (mode === 'extraMain' || mode === 'withdraw') && !!form.querySelector('[name="affectsBudget"]')?.checked;
  ...
  openSalary() {
    ...
    const first = Data.firstMonth();
    const body = `
      <form class="form" data-form="salary" novalidate>
        <p class="section-sub">سجّل اليوم الذي نزل فيه راتبك فعليًا، ويبدأ الشهر المالي الجديد وحساب الميزانية اليومية منه.</p>
        ${C.dateField({ name: 'date', label: 'تاريخ نزول الراتب', value: today, min: first ? first.start : '', max: today })}
        ${C.amountField({ name: 'salary', label: 'مبلغ الراتب', value: state.settings.salary, big: true })}
        <div class="callout" data-salary-plan>${Utils.icon('calendar')}<p></p></div>
        ${C.formActions('تأكيد')}
      </form>`;
    UI.modal({ title: 'نزل الراتب', body });
    const form = document.querySelector('.modal-layer:last-child form[data-form="salary"]');
    const input = form?.querySelector('[name="date"]');
    const update = () => {
      const p = form.querySelector('[data-salary-plan] p');
      const d = input.value;
      if (!DateUtils.isValidISO(d)) { p.textContent = ''; return; }
      const plan = Months.salaryPlan(d);
      if (!plan) { p.textContent = ''; return; }
      if (plan.type === 'same') p.textContent = `الشهر الحالي يبدأ بالفعل في ${DateUtils.long(d)}، سيتم تحديث مبلغ الراتب فقط.`;
      else if (plan.type === 'late') p.textContent = `راتب متأخر: سيبدأ الشهر الحالي من ${DateUtils.long(d)} بدل ${DateUtils.long(plan.month.start)}، ويمتد الشهر السابق حتى ${DateUtils.long(DateUtils.addDays(d, -1))}.`;
      else p.textContent = `سينتهي الشهر الحالي في ${DateUtils.long(DateUtils.addDays(d, -1))}، ويبدأ شهر جديد من ${DateUtils.long(d)} حتى ${DateUtils.long(plan.end)} (قابل للتعديل من صفحة الشهور).`;
    };
  ...
  submitSalary(form) {
    ...
    const first = Data.firstMonth();
    const { values, ok } = Forms.validate(form, {
      date: (v) => Forms.date(v, { min: first ? first.start : '', max: today }),
      salary: (v) => Forms.amount(v),
    });
    if (!ok) return;
    const snapshot = JSON.stringify(state.months);
    const err = Months.applySalary(values.date, values.salary);
    if (err) {
      state.months = JSON.parse(snapshot);
      Forms.showErrors(form, { date: err });
      return;
    }
    Store.save('months');
    Months.ensure();
    UI.closeAllModals();
    const m = Data.monthForDate(values.date);
    UI.toast(m ? `بدأ الشهر المالي ${DateUtils.range(m.start, m.end)}` : 'تم تسجيل الراتب');
    App.render();
  },
  ...
  openMonthEdit(month) {
    const body = `
      <form class="form" data-form="month" novalidate>
        ...
        <div class="grid-2">
          ${C.dateField({ name: 'start', label: 'تاريخ نزول راتب هذا الشهر', value: month.start })}
          ${C.dateField({ name: 'nextPay', label: 'تاريخ نزول الراتب القادم', value: DateUtils.addDays(month.end, 1) })}
        </div>
        <p class="field-hint" style="margin-top:-6px">الشهر ينتهي تلقائيًا قبل الراتب القادم بيوم، ويُعدَّل الشهر السابق واللاحق ليتوافقا.</p>
  ...
  submitMonth(form) {
    const { values, ok } = Forms.validate(form, {
      ...
      start: (v) => Forms.date(v),
      nextPay: (v) => Forms.date(v),
    });
    if (!ok) return;
    const month = Data.monthById(form.querySelector('[name="id"]').value);
    if (!month) return;
    const snapshot = JSON.stringify(state.months);
    const err = Months.setBounds(month, values.start, DateUtils.addDays(values.nextPay, -1));
    if (err) {
      state.months = JSON.parse(snapshot);
      Forms.showErrors(form, { nextPay: err });
      return;
    }
    const { start, nextPay, ...rest } = values;
    Object.assign(month, rest);
    Months.ensure();
    Store.save('months');
    UI.closeAllModals();
...
const Actions = {
  ...
  'salary-arrived'() { Forms.openSalary(); },
